...
if __name__ == "__main__":
    ...
    DEFAULT_CONFIG = {
        ...
        # Policy initialization params
        'policy_init': 'none',  # {'none', 'warm_start', 'kl_reg'}
        'bc_warmstart_steps': 2000,
        'bc_batch_size': 1024,
        'bc_collect_steps': 4096,
        'bc_freeze_value': True,
        'bc_temperature': 1.0,

        # Critic initialization params
        'critic_warmstart_steps': 0,
        'critic_warmstart_collect': 4096,
        'critic_warmstart_calibrate': True,
        'critic_warmstart_rollouts_per_state': 3,
        'critic_warmstart_horizon': 8,

        # Kickstarting (teacher KL) params
        'kickstart_alpha_initial': 1.0,
        'kickstart_alpha_final': 0.0,
        'kickstart_warmup_steps': 0,
        'kickstart_anneal_steps': 300000,
        'kickstart_temperature': 1.0,
        'kickstart_kl_direction': 'forward',
        'kickstart_eps': 1e-8,
    ...
    def build_namespace(config):
        ...
        direction_raw = cfg.get('kickstart_kl_direction', 'forward')
        if direction_raw is None:
            cfg['kickstart_kl_direction'] = 'forward'
        else:
            direction_norm = str(direction_raw).strip().lower()
            allowed_dirs = {'forward', 'reverse'}
            if direction_norm not in allowed_dirs:
                allowed_str = ", ".join(sorted(allowed_dirs))
                raise ValueError(
                    f"Unsupported kickstart_kl_direction '{direction_raw}'. Allowed values: {allowed_str}."
                )
            cfg['kickstart_kl_direction'] = direction_norm

        cfg['kickstart_alpha_initial'] = float(cfg.get('kickstart_alpha_initial', 1.0))
        cfg['kickstart_alpha_final'] = float(cfg.get('kickstart_alpha_final', 0.0))
        cfg['kickstart_warmup_steps'] = int(cfg.get('kickstart_warmup_steps', 0))
        cfg['kickstart_anneal_steps'] = int(cfg.get('kickstart_anneal_steps', 300000))
        cfg['kickstart_temperature'] = float(cfg.get('kickstart_temperature', 1.0))
        cfg['kickstart_eps'] = float(cfg.get('kickstart_eps', 1e-8))

        policy_init_raw = cfg.get('policy_init', 'none')
        if not isinstance(policy_init_raw, str):
            raise ValueError("policy_init must be a string value.")
        policy_init = policy_init_raw.strip().lower()
        allowed_policy_inits = {'none', 'warm_start', 'kl_reg'}
        if policy_init not in allowed_policy_inits:
            allowed = ", ".join(sorted(allowed_policy_inits))
            raise ValueError(
                f"Unsupported policy_init '{policy_init_raw}'. Allowed values: {allowed}."
            )
        cfg['policy_init'] = policy_init

        cfg['bc_warmstart_steps'] = max(0, int(cfg.get('bc_warmstart_steps', 0)))
        cfg['bc_batch_size'] = max(1, int(cfg.get('bc_batch_size', 1)))
        cfg['bc_collect_steps'] = max(0, int(cfg.get('bc_collect_steps', 0)))
        cfg['bc_freeze_value'] = bool(cfg.get('bc_freeze_value', True))
        cfg['bc_temperature'] = float(cfg.get('bc_temperature', 1.0))

        cfg['critic_warmstart_steps'] = max(0, int(cfg.get('critic_warmstart_steps', 0)))
        cfg['critic_warmstart_collect'] = max(0, int(cfg.get('critic_warmstart_collect', 0)))
        cfg['critic_warmstart_calibrate'] = bool(cfg.get('critic_warmstart_calibrate', True))
        cfg['critic_warmstart_rollouts_per_state'] = max(1, int(cfg.get('critic_warmstart_rollouts_per_state', 1)))
        cfg['critic_warmstart_horizon'] = max(1, int(cfg.get('critic_warmstart_horizon', 1)))

        if policy_init == 'kl_reg':
            current_strategy = cfg.get('kge_integration_strategy')
            if current_strategy not in (None, 'kickstart'):
                raise ValueError(
                    "policy_init='kl_reg' requires kge_integration_strategy to be None or 'kickstart'."
                )
            cfg['kge_integration_strategy'] = 'kickstart'
    ...
    for config in run_configs:
        ...
        run_vars = (
            ...
            args_namespace.policy_init,
            args_namespace.critic_warmstart_steps,
