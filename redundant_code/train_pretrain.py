...
def _build_callbacks(
    ...
    should_critic_warmstart = (
        args.critic_warmstart_steps > 0
        and args.timesteps_train > 0
        and not args.load_model
    )
    if should_critic_warmstart:
        print("[Init] Critic warm-start (fit V to KGE potential)...")
        critic_warmstart(
            model=model,
            callback_env=callback_env,
            index_manager=index_manager,
            kge_engine=kge_engine,
            steps=args.critic_warmstart_steps,
            collect_steps=args.critic_warmstart_collect,
            calibrate=args.critic_warmstart_calibrate,
            rollouts_per_state=args.critic_warmstart_rollouts_per_state,
            horizon=args.critic_warmstart_horizon,
        ...
        try:
            ...
            print(f"[Critic warmstart] Warning: could not reset callback env ({exc}).")
...
def main(args, log_filename, use_logger, use_WB, WB_path, date):
    ...
    policy_kwargs = {
        ...
        'kickstart_alpha_initial': args.kickstart_alpha_initial,
        'kickstart_alpha_final': args.kickstart_alpha_final,
        'kickstart_warmup_steps': args.kickstart_warmup_steps,
        'kickstart_anneal_steps': args.kickstart_anneal_steps,
        'kickstart_temperature': args.kickstart_temperature,
        'kickstart_kl_direction': args.kickstart_kl_direction,
        'kickstart_eps': args.kickstart_eps,
    }
    ...
    should_warmstart = (
        args.policy_init in {"warm_start", "kl_reg"}
        and args.bc_warmstart_steps > 0
        and args.timesteps_train > 0
        and not args.load_model
    )
    if should_warmstart:
        print("[Init] Policy warm-start using KGE teacher...")
        policy_bc_warmstart(
            model,
            callback_env,
            index_manager,
            ...
            collect_steps=args.bc_collect_steps,
            freeze_value=args.bc_freeze_value,
            temperature=args.bc_temperature,
            verbose=1,
        )
        # Reset callback env so evaluation callbacks start from a clean state
        try:
            callback_env.reset()
        except Exception as exc:
            print(f"[BC warmstart] Warning: could not reset callback env ({exc}).")
