...
from str_utils import Term, Rule
...
def get_next_unification_python(state: List[Term],
    ...
    # Sort state to handle more constrained goals first (fewer variables), which can prune the search tree faster.
    ...
    for subs in fact_substitutions:
        ...
        if subs.get('True') == 'True': # Ground query matched a fact
            ...
            # NOTE: We do NOT use excluded_fact here because these are remaining goals after
            # fact unification, not the original query
            for j in range(len(new_state)):
                atom = new_state[j]
                if atom in facts_set:
    ...
    for body, subs in rule_results:
        ...
        # NOTE: We do NOT use excluded_fact here because atoms generated by rule application
        # should be checked against ALL facts. The exclusion only applies to direct unification
        # of the original query with facts.
        for j in range(len(new_state)):
            atom = new_state[j]
            if atom in facts_set:
                new_state[j] = Term('True', ())
    ...
    if strategy == 'complete':
        ...
        for r_state in rule_derived_states:
            ...
            for subs in r_fact_substitutions:
                ...
                # substitute the facts by True
                # NOTE: We do NOT use excluded_fact here because these are goals after
                # rule+fact unification, not the original query
                for j in range(len(new_state)):
                    atom = new_state[j]
                    if atom in facts_set:
                        new_state[j] = Term('True', ())
