...
def get_negatives(
    ...
    """
    Generate `num_negs` negatives per positive in a single CUDA kernel call +
    one membership test.  No Py-level book-keeping, no per-triple filtering.
    """
    B = sub_indices.size(0)
    rels, heads, tails = sub_indices[:, 0, 0], sub_indices[:, 0, 1], sub_indices[:, 0, 2]
    pos_batch = torch.stack([heads, rels, tails], dim=1)                        # (B, 3)

    overshoot = 3                                                               # ↓ a bit
    cand = self.corrupt_batch(
        pos_batch, num_negs_per_pos=overshoot * num_negs
    ).view(-1, 3)

    if self.filterer is not None:
        cand = cand.to(self.filterer._hashes_sorted.device)     # ← was _hashes.device
        keep = self.filterer(cand.unsqueeze(0)).squeeze(0)
        cand = cand[keep]

    cand = cand[: B * num_negs].view(B, num_negs, 3)                            # trim

    # ---- pack back to (B, k, padding_atoms, max_arity+1) ---------------------
    neg_subs = torch.full(
        (B, num_negs, padding_atoms, max_arity + 1),
        self.index_manager.padding_idx,
        dtype=torch.long,
        device=device,
    )
    neg_subs[:, :, 0, 1] = cand[:, :, 0]   # head
    neg_subs[:, :, 0, 0] = cand[:, :, 1]   # relation
    neg_subs[:, :, 0, 2] = cand[:, :, 2]   # tail
    return neg_subs
