...
from sb3.sb3_unification import state_to_str
...
def run_sb3_env_single_query(
    ...
    # Use the Term class from the loaded facts to avoid class mismatch issues
...
def run_sb3_env(
    ...
    for idx, (split, query_tuple) in enumerate(queries):
        ...
        # Setup query - use the Term class from loaded facts to avoid class mismatch
        TermClass = type(dh_str.facts[0]) if dh_str.facts else StrTerm
        q_str = TermClass(predicate=p, args=(h, t))
