...
def _unify_with_facts(
    ...
    # Next: determine which matches lead to immediate proofs-------------------- 
...
def _unify_with_rules(
    ...
    if A == 0 or engine.rules_heads_sorted.numel() == 0:
        empty = torch.full((A, 0, 1, 3), pad, dtype=torch.long, device=device)
        if verbose > 1 and engine.debug_helper:
            print(f"\n_unify_with_rules: Early exit - A={A}, no rules")
        return empty, torch.zeros(A, dtype=torch.long, device=device)
...
class UnificationEngine:
    ...
    def __init__(self,
        ...
        # Compute max rule body size for determining output state size
        ...
        # helper tensors for TRUE/FALSE/END atoms
    ...
    def get_derived_states(self,
        ...
        """
        if verbose > 0: print(f"{'*' * 10}")
        if verbose > 1:
            caller_info = traceback.extract_stack()[-2]
            print(f"[DEBUG get_derived_states] Called from {caller_info.filename}:{caller_info.lineno} in {caller_info.name}, "
                  f"B={current_states.shape[0]}, next_var_indices={next_var_indices.tolist() if current_states.shape[0] <= 3 else 'many'}")
        ...
        max_atoms_output = max(max_atoms + self.max_rule_body_size, 10)
        ...
        # Proceed with active states only
        active = ~terminal
        ...
        rule_states, rule_counts = _unify_with_rules(
            ...
            max_output_per_query=max_derived_per_state,verbose=verbose
        )
        ...
        # ---- 3) facts
        if verbose > 1:
            print(f"\nCalling _unify_with_facts with A={A}")
            if excluded_queries is not None:
        ...
        if not owner_mask.any():
            if verbose > 1:
                print(f"[ENGINE] [DEBUG EARLY RETURN 1] No owner_mask, immediate_idx={immediate_idx.numel()}, no_match={(~has_any_unification).sum().item()}/{A}")
        ...
        if verbose > 1:
            print(f"\n{'='*60}")
            print("[ENGINE] 4. PRUNE GROUND FACTS & DETECT IMMEDIATE PROOFS")
            print(f"{'='*60}")
        ...
        if not keep_mask.any():
            if verbose > 1:
                print(f"[ENGINE] [DEBUG EARLY RETURN 2] No keep_mask, is_proof={is_proof.sum().item()}/{is_proof.numel()}, immediate={immediate_idx.numel()}/{A}")
            return (all_derived, derived_counts, updated_next_var_indices)
        ...
        # IMPORTANT: use the *adjusted* next-var indices (which already account for
        # variables present in the current state) so canonicalization shares the same
        # base as rule/fact instantiation. Using the stale `next_var_indices` causes
        # each derived state to start from different Var IDs compared to the string env.
        if owners_kept.numel() > 0:
            next_vars_for_states = updated_next_var_indices[owners_kept]
        else:
            next_vars_for_states = next_var_indices.new_empty((0,), dtype=next_var_indices.dtype)
        if verbose > 1:
            print(f"[ENGINE] [DEBUG CANON] canonical_action_order={self.canonical_action_order}, num_states={states_kept.shape[0] if states_kept.numel() > 0 else 0}, tensor_size={states_kept.numel()}, debug_helper={self.debug_helper is not None}")
        if self.canonical_action_order and states_kept.numel() > 0:
            # DEBUG: Print first few states before canonical ordering ALWAYS
            if verbose > 1:
                print(f"\n[ENGINE] [BATCHED DEBUG] Before canonical ordering: {states_kept.shape[0]} states")
                num_to_print = min(3, states_kept.shape[0])
                for i in range(num_to_print):
                    ...
                    if valid.any():
                        atoms = state[valid]
                        atoms_str = [self.debug_helper.atom_to_str(atom) for atom in atoms]
                        key = self.debug_helper._tensor_state_canonical_key(state, self.constant_no, pad)
                        # print(f"  State {i}: {' | '.join(atoms_str)}")
                        # print(f"    Key: {key[:15]}...")
            
            states_kept, counts_kept, owners_kept, next_vars_for_states = self.debug_helper._sort_candidates_by_canonical_order(
                states_kept, counts_kept, owners_kept, next_vars_for_states,
                self.constant_no, pad
            )
            
            if verbose > 1:
                print(f"[ENGINE] [BATCHED DEBUG] After canonical ordering:")
                num_to_print = min(3, states_kept.shape[0])
                for i in range(num_to_print):
                    state = states_kept[i]
                    valid = state[:, 0] != pad
        ...
        canon_states, next_vars_per_state = self._canonicalize_variables(
            ...
            next_vars_for_states, owners_kept, pad
        )
        
        # v) Print after canonicalization (before dedup)
        if verbose > 1:
            print(f"\n{'='*60}")
            print("[ENGINE] CANONICALIZE (before dedup)")
            print(f"{'='*60}")
            if verbose > 1:
        ...
        if owners_kept.numel() > 0:
            ...
            # Filter out states beyond K (don't clamp, actually remove them)
            keep_mask = pos < K
            pos = pos[keep_mask]
            dst_b = sort_vals[keep_mask]
            dst_s = pos
            states_sorted = states_sorted[keep_mask]
            counts_sorted = counts_sorted[keep_mask]
        ...
        if verbose > 0:
            self.debug_helper.print_states(f"5. FINAL NEXT STATES{dedup_suffix}", unique_states, unique_counts)
    ...
    def _canonicalize_variables(self,
        ...
        """
        Canonicalize *newly introduced* runtime variables per state so they follow
        the same numbering scheme as the string environment:
          - Existing runtime vars (those with indices < next_var_start[state]) are left untouched.
          - New vars (indices ≥ next_var_start[state]) are renamed deterministically based on
            first occurrence order, always starting from next_var_start[state].
        ...
        # Identify runtime variable slots
        is_var = (args > constant_no) & (args != pad)
        if not is_var.any():
            # No variables at all
            return states, next_var_start

        # Broadcast next_var_start to argument grid
        base = next_var_start.view(N, 1, 1)
        is_new_var = is_var & (args >= base)
        if not is_new_var.any():
            # Only existing vars, nothing to rename
            return states, next_var_start

        # Linear position per arg slot (0..M*2-1), used to order first occurrences deterministically
        # Match string engine ordering: iterate atoms in sequence and visit arg0, then arg1
        # so first occurrences are tracked exactly as rename_vars_local does.
        lin_pos = torch.arange(M * 2, device=device).view(1, M, 2).expand(N, -1, -1).clone()

        occ_state = torch.arange(N, device=device).view(N, 1, 1).expand_as(args)
        SHIFT = self.runtime_var_end_index + 2  # > any index to keep (state, var_id) pairs unique
        combined = args + occ_state * SHIFT

        occ_keys = combined[is_new_var]
        occ_lin = lin_pos[is_new_var]
        if occ_keys.numel() == 0:
            return states, next_var_start

        # Group occurrences of the same variable within a state
        ...
        first_pos_per_group = lin_sorted[seg_start]

        group_state = (uniq // SHIFT).long()
        vars_per_state = torch.bincount(group_state, minlength=N)
        if owners.numel() > 0:
            owner_max = int(owners.max().item()) + 1
            owner_offsets = torch.zeros(owner_max, dtype=next_var_start.dtype, device=device)
            owner_offsets.scatter_reduce_(0, owners, vars_per_state, reduce='amax', include_self=False)
            offset_per_state = owner_offsets.index_select(0, owners)
        else:
            offset_per_state = torch.zeros_like(next_var_start)
        effective_base_per_state = next_var_start + offset_per_state
        ...
        if state_group_id.numel() > 0:
            num_state_groups = int(state_group_id.max().item()) + 1
            group_starts = torch.full((num_state_groups,), len(state_group_id),
                                      dtype=torch.long, device=device)
            positions = torch.arange(len(state_group_id), device=device)
            group_starts.scatter_reduce_(0, state_group_id, positions, reduce='amin', include_self=False)
            rank_sorted = positions - group_starts[state_group_id]
        else:
            rank_sorted = state_group_id
        ranks = torch.empty_like(rank_sorted)
        ranks[ord3] = rank_sorted

        base_per_group = effective_base_per_state.index_select(0, group_state)
        new_id_per_group = base_per_group + ranks
        if new_id_per_group.numel() > 0:
            overflow_groups = new_id_per_group > self.runtime_var_end_index
        ...
        canon_states = states.clone()
        canon_states[:, :, 1:3][is_new_var] = new_id_per_occ.long()

        updated_next = effective_base_per_state + vars_per_state
    ...
    def is_terminal_state(self, states: Tensor) -> Tensor:
        ...
        if self.true_pred_idx is not None:
            ...
            is_terminal |= all_true
        
        # Check for FALSE
        if self.false_pred_idx is not None:
            any_false = (preds == self.false_pred_idx).any(dim=1)
            is_terminal |= any_false
        
        # Check for END (if end_proof_action is enabled)
        if self.end_proof_action and self.end_pred_idx is not None:
            single_pred = non_pad.sum(dim=1) == 1
            first_pos = non_pad.long().argmax(dim=1)
            batch_ids = torch.arange(states.shape[0], device=self.device)
            first_pred = preds[batch_ids, first_pos]
            is_end = single_pred & (first_pred == self.end_pred_idx)
            is_terminal |= is_end
        
        return is_terminal

    def pad_state(self, state: Tensor, target_atoms: int) -> Tensor:
        """
        Pad a state tensor to have exactly target_atoms rows.
        
        Args:
            state: [M, 3] tensor to pad
            target_atoms: Desired number of atoms
            
        Returns:
            [target_atoms, 3] tensor padded with padding_idx
        """
        D = self.max_arity + 1
        
        # Ensure state is on correct device and dtype
        if state.device != self.device:
            state = state.to(device=self.device)
        if state.dtype != torch.long:
            state = state.long()
            
        if state.shape[0] >= target_atoms:
            return state[:target_atoms]
        
        pad_rows = target_atoms - state.shape[0]
        tail = torch.full((pad_rows, D), self.padding_idx, dtype=torch.long, device=self.device)
        return torch.cat([state, tail], dim=0)

    def create_false_state(self, target_atoms: int) -> Tensor:
        """
        Create a state containing only the False predicate, padded to target_atoms.
        
        Args:
            target_atoms: Number of atoms in the padded state
            
        Returns:
            [target_atoms, 3] tensor with False predicate in first row
        """
        if self.false_pred_idx is None:
            raise ValueError("False predicate index is not defined.")
        
        state = torch.full((1, self.max_arity + 1), self.padding_idx, 
                          dtype=torch.long, device=self.device)
        state[0, 0] = int(self.false_pred_idx)
        return self.pad_state(state, target_atoms)

    def create_end_state(self, target_atoms: int) -> Tensor:
        """
        Create a state containing only the End predicate, padded to target_atoms.
        
        Args:
            target_atoms: Number of atoms in the padded state
            
        Returns:
            [target_atoms, 3] tensor with End predicate in first row
        """
