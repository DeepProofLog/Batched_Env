...
class FastMaskedCategorical:
    ...
    def sample(self) -> Tensor:
        """Sample actions from the distribution (fullgraph-compatible)."""
        # Compute probabilities if not cached
        if self.probs is None:
            self.probs = self._log_probs.exp()
        
        probs = self.probs
        
        # Vectorized safety: handle rows where all actions are masked (sum=0)
        # Using purely tensor operations to avoid data-dependent branching
        # which breaks torch.compile fullgraph mode
        sum_probs = probs.sum(dim=-1, keepdim=True)
        mask = (sum_probs == 0)  # [B, 1]
        
        # Add uniform distribution to fully-masked rows
        # probs_safe = probs + mask * (1.0 / num_actions)
