...
class KGEInference:
    ...
    persist_runtime_scores: bool = True,
        ...
        if runtime_cache_max_entries == 0:
            ...
            self._runtime_cache_max_entries: Optional[int] = 0
            self._score_cache: Optional[OrderedDict[str, float]] = None
        else:
            self._runtime_cache_enabled = True
            if runtime_cache_max_entries is not None and runtime_cache_max_entries > 0:
                self._runtime_cache_max_entries = runtime_cache_max_entries
            else:
                self._runtime_cache_max_entries = None
    ...
    def _store_runtime_score(self, atom_str: str, score: float) -> None:
        if self._runtime_cache_enabled and self._score_cache is not None:
            ...
            cache[atom_str] = score
            cache.move_to_end(atom_str)
            if (
                self._runtime_cache_max_entries is not None
                and len(cache) > self._runtime_cache_max_entries
            ):
    ...
    def predict(self, atom_string: str) -> float:
        ...
        cached = self.atom_scores.get(atom_string)
        if cached is not None:
            return cached

        runtime_cached = self._get_runtime_cached_score(atom_string)
        ...
        score = float(atom_outputs[0][0].numpy())
        self._store_runtime_score(atom_string, score)
        return score
    ...
    def predict_batch(self, atoms_for_ranking: Sequence[Union[str, Tuple]]) -> List[float]:
        ...
        for idx, atom_str in enumerate(canonical_atoms):
            ...
            if cached_score is None:
                cached_score = self._get_runtime_cached_score(atom_str)
        ...
        if missing:
            ...
            for atom_str, score in zip(unique_missing, new_scores):
                ...
                newly_scored[atom_str] = score_float
                self._store_runtime_score(atom_str, score_float)

            for idx, atom_str in zip(missing_indices, missing):
                cached_score = self.atom_scores.get(atom_str)
                if cached_score is None:
                    cached_score = newly_scored.get(atom_str)
                if cached_score is None:
                    cached_score = self._get_runtime_cached_score(atom_str)
                if cached_score is None:
                    raise RuntimeError(f"Missing score for atom '{atom_str}' after model evaluation.")
                scores[idx] = float(cached_score)
