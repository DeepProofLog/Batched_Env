...
class BatchLogicProofEnv(EnvBase):
    ...
    def get_constants_predicates(self, rules: List[Rule]) -> Tuple[set, set]:
        ...
        for rule in rules:
            ...
            constants.update([str(arg) for arg in rule.head.args if not is_variable(arg) and not re.search(r'\[', arg)])
            # get from body the predicates and the constants if they are not variables
            for atom in rule.body:
                predicates.add(str(atom.predicate))
                constants.update([str(arg) for arg in atom.args if not is_variable(arg) and not re.search(r'\[', arg)])
        predicates.remove("proof_first")
        predicates.remove("call")
        # remove from constants if it has an espcial character such as [ 
        return constants, predicates
    
    def create_global_idx(self):
        '''Create a global index for a list of terms. Start idx counting from 1'''
        rules = self.get_rules_from_file(self.knowledge_f)
        constants, predicates = self.get_constants_predicates(rules)
        self.constant_str2idx = {term: i+1 for i, term in enumerate(constants)}
        self.predicate_str2idx = {term: i+1 for i, term in enumerate(predicates)}
