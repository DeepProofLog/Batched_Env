...
class Sampler:
    ...
    def _filter_keep_mask(self, triples: LongTensor) -> torch.BoolTensor:
        ...
        # Binary search
        idx = torch.searchsorted(self.hashes_sorted, hashes)
        # Clamp idx to valid range to prevent out-of-bounds access for hashes larger than any in hashes_sorted
        idx = idx.clamp(max=self.hashes_sorted.numel() - 1)
        found = self.hashes_sorted[idx] == hashes
        
        # Debug: Check for specific hash corresponding to locatedInCR(bulgaria, oceania)
        # r=1, h=37, t=178 -> hash = (37<<42) | (1<<21) | 178
        # We need to check if this hash is being computed and found.
        # This is a bit hacky but useful for debug.
        # h=37, r=1, t=178
        target_hash = (37 << 42) | (1 << 21) | 178
        if (hashes == target_hash).any():
            print(f"\n!!! DEBUG: target_hash {target_hash} FOUND in candidates!")
            is_found = found[hashes == target_hash]
            print(f"   Is it in hashes_sorted? {is_found}")
            if is_found.any():
                idx_t = idx[hashes == target_hash]
                print(f"   Index in sorted: {idx_t}")
                print(f"   Value at index: {self.hashes_sorted[idx_t]}")
    ...
    def corrupt(
        ...
        *,
        ...
        # DISABLED for parity with SB3: SB3 returns fewer negatives if valid ones are exhausted.
