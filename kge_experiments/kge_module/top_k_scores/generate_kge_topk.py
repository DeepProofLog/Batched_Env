...
from kge_module import KGEInference, find_latest_run, normalize_backend  # your engine
...
def infer_run_signature(
    ...
    if run_signature is None:
        raise ValueError(
            f"No checkpoint found for dataset '{dataset_name}' under {checkpoint_dir}. "
            "Please provide --run-signature."
        )
...
def main():
    ...
    parser.add_argument("--backend", default="torch", choices=["torch", "pykeen"], help="Backend used by KGEInference.")
    ...
    run_signature = infer_run_signature(
        args.dataset,
        args.run_signature,
        args.checkpoint_dir,
        args.backend,
