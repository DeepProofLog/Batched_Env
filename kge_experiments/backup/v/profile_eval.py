...
def create_ppo_and_warmup(components, config) -> Tuple[any, float]:
    ...
    ppo = PPO(policy, env, config, device=env.device, batch_size_env=batch_size)
    ...
    # Trigger JIT compilation with exact batch size using the correct method
    warmup_sampler = components['sampler']
    warmup_queries = queries[:1]
    
    # Run a tiny evaluation to force compilation of the specific method's step function
    _ = ppo.run_evaluation(
        queries=warmup_queries,
        sampler=warmup_sampler,
        n_corruptions=1, # Minimal work for warmup
...
def main():
    ...
    parser.add_argument('--eval-method', type=str, default='persistent', 
                        choices=['standard', 'batched', 'persistent', 'streaming'],
