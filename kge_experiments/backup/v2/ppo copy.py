...
from env import EnvVec, TensorDictEnvWrapper

if torch.cuda.is_available():
    torch.set_float32_matmul_precision('high')

from rollout import RolloutBuffer
from env import EnvVec, EnvObs, EnvState, TensorDictEnvWrapper
...
class PPOOptimal:
    ...
    def _setup_fused_eval_step(self):
        ...
        if not self.parity:
            self._compiled_eval_step = torch.compile(fused_eval, mode=_compile_mode, fullgraph=True)
    ...
    # -------------------------------------------------------------------------
    # PARITY EVALUATION - eval_corruptions style with proper slot management
    # -------------------------------------------------------------------------

    def _setup_parity_eval_step(self, compile_mode: str = 'default'):
        ...
        if hasattr(self, cache_key):
            return (getattr(self, cache_key), 
                    getattr(self, f'_parity_reset_{compile_mode}'),
                    getattr(self, f'_parity_loop_step_{compile_mode}'))
            
        policy = self._uncompiled_policy
        main_env = self.env.env if hasattr(self.env, 'env') and not isinstance(self.env, EnvVec) else self.env
        device = self.device
        ...
        if compile_mode == 'eager' or self.parity:
        ...
        setattr(self, cache_key, step_fn)
        setattr(self, f'_parity_reset_{compile_mode}', reset_fn)
        setattr(self, f'_parity_loop_step_{compile_mode}', loop_step_fn)
    ...
    def evaluate_parity(
        ...
        Implements the same evaluation protocol as tensor_model_eval.eval_corruptions:
        - Per-slot episode tracking with per_slot_lengths
        - Log probability accumulation per episode  
        - Vectorized ranking with RNG tie-breaking
        
        For production use with slot-recycling, use evaluate().
        
        Args:
            queries: [N, 3] tensor of positive queries (unpadded)
            sampler: Sampler for generating corruptions
            n_corruptions: Number of corruptions per query per mode
            corruption_modes: ('head', 'tail', 'both')
            query_depths: Optional [N] depths for stats
            verbose: Print progress
            deterministic: Use argmax (True) or sample (False) actions
            compile_mode: 'default' for fast compiled, 'eager' for parity tests
            
        Returns:
            Dict with MRR, Hits@K metrics matching eval_corruptions output format
        """
        # Setup compiled functions
        parity_step_fn, parity_reset_fn, parity_loop_step_fn = self._setup_parity_eval_step(compile_mode)
        ...
        per_mode_ranks: Dict[str, List[Tensor]] = {m: [] for m in corruption_modes}
        ...
        for start in range(0, N, B):
            ...
            for mode in corruption_modes:
                ...
                def get_slot_queries(ep_counts: Tensor) -> Tensor:
                ...
                max_steps = T * self.max_depth * 2  # Conservative bound
        ...
        self._uncompiled_policy.train()
        
        # Aggregate metrics
        def compute_metrics(ranks: Tensor) -> Dict[str, float]:
            if ranks.numel() == 0:
                return {"MRR": 0.0, "Hits@1": 0.0, "Hits@3": 0.0, "Hits@10": 0.0}
            r = ranks.float()
            return {
