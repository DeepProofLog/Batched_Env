"""Profile minimal eval V7 - manual CUDA graph capture."""
import os
import sys
...
def setup(device, config):
    ...
    from engine_minimal_v2 import EngineMinimalV2
    from eval_minimal_v7 import MinimalEvalV7
    ...
    engine = EngineMinimalV2.from_vectorized_engine(
        vec_engine, 
        max_derived=config.padding_states,
    ...
    policy = ActorCriticPolicy(
        ...
        hidden_dim=256, num_layers=8, dropout_prob=0.0, device=device, compile_policy=False,  # Don't compile policy separately
    ).to(device)
    
    evaluator = MinimalEvalV7(
        policy=policy, engine=engine, batch_size=config.batch_size,
        padding_atoms=config.padding_atoms, padding_states=config.padding_states,
        max_depth=config.max_depth, device=device,
    )
    
    return {'eval': evaluator, 'sampler': sampler, 'queries': queries}
...
def main():
    ...
    print(f"Minimal Eval V7 | Candidates: {total}, Batch: {config.batch_size}")
    ...
    print("Warmup...")
    t0 = time()
    # Just run one small eval - graph will be captured on demand
    c['eval'].evaluate(c['queries'][:5].to(device), c['sampler'], n_corruptions=5, corruption_modes=('head',))
    ...
    print(f"ms/candidate: {ms_cand:.4f}  (target: 0.3)")
