"""Profile minimal eval V4 with single-graph compilation."""
import os
import sys
...
def setup(device, config):
    ...
    from eval_minimal_v4 import MinimalEvalV4
    ...
    # Create full engine first (needed for correct initialization)
    ...
    # Create minimal engine from full engine
    engine = EngineMinimal.from_vectorized_engine(
        vec_engine, 
...
def main():
    ...
    print(f"Minimal Eval V4 | Candidates: {total}, Batch: {config.batch_size}")
    ...
    _ = c['eval'].evaluate(c['queries'][:5].to(device), c['sampler'], n_corruptions=5, corruption_modes=('head',))
