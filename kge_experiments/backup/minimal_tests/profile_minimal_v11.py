"""Profile minimal eval V11 - Best performing version."""
import os
import sys
...
def setup(device, config):
    ...
    from eval_minimal_v11 import MinimalEvalV11
    ...
    # Use EngineWrapper for clean interface
    engine = EngineWrapper.from_vectorized_engine(
        vec_engine, 
...
def main():
    ...
    print(f"Minimal Eval V11 | Candidates: {total}, Batch: {config.batch_size}")
    ...
    t0 = time()
    c['eval'].engine.compile(mode='reduce-overhead')  # Compile engine (4.3x faster)
