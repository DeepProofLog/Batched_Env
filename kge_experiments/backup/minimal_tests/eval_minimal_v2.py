"""
Minimal Standalone Evaluation V2 - Fused step+reset in compiled function.

Key optimization: Include reset logic INSIDE the compiled step function.
"""
import torch
...
class MinimalEvalV2:
    """Minimal eval with reset INSIDE compiled step."""
    ...
    def _init_from_pool(self):
        """Initialize first B slots from pool."""
        B, A, S, pad = self.B, self.A, self.S, self.pad
        
        # Fetch first B candidates
        init_idx = torch.arange(B, device=self.device).clamp(max=self._pool_size - 1)
        queries_raw = self._pool[init_idx]  # [B, 3]
    ...
    def compile(self, mode: str = 'reduce-overhead'):
        """Compile step that returns results."""
        policy = self.policy
        engine = self.engine
        ...
        # Capture pool via closure
        eval_obj = self
        
        def step_and_reset() -> Tuple[Tensor, Tensor, Tensor]:
            """Step + reset done slots. Returns (newly_done, success, indices)."""
            # Get actions
            ...
            K, M = min(reset_derived_raw.shape[1], S), min(reset_derived_raw.shape[2], A)
            reset_derived[:, :K, :M, :] = reset_derived_raw[:, :K, :M, :]
            
            within = arange_S.unsqueeze(0) < reset_counts_raw.unsqueeze(1)
            valid = reset_derived[:, :, :, 0] != pad
            ac = valid.sum(2)
            base_valid = within & (ac <= A) & (ac > 0)
            reset_derived = torch.where(base_valid.unsqueeze(-1).unsqueeze(-1), reset_derived,
                                       false_state.unsqueeze(0).expand(B,-1,-1,-1))
            reset_counts = base_valid.sum(1)
            reset_counts = torch.where(reset_counts == 0, ones_B, reset_counts)
            
            # Merge: use reset values for needs_reset slots
            m1 = needs_reset.view(B, 1, 1)
            m3 = needs_reset.view(B, 1, 1, 1)
            
            final_current = torch.where(needs_reset.view(B, 1, 1), reset_queries, new_current)
        ...
        print(f"[MinimalEvalV2] Compiled (mode={mode})")
    ...
    def evaluate(
        ...
        while steps < max_steps:
            ...
            # Record results (vectorized scatter)
