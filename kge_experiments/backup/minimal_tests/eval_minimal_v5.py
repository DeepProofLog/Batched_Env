"""
Minimal Eval V5 - Single graph compilation, no copy_ mutations inside.

Key: Return new tensors from compiled step, copy OUTSIDE.
"""
import torch
...
class MinimalEvalV5:
    """Eval with step+reset compiled, mutations outside."""
    ...
    def __init__(
        ...
        # State buffers (mutable, copied to OUTSIDE compiled step)
    ...
    def compile(self, mode: str = 'reduce-overhead'):
        """Compile step+reset - RETURNS new tensors, no mutations inside."""
        policy = self.policy
        engine = self.engine
        ...
        def step_and_reset(
            ...
            """
            Pure functional step + reset.
            
            Returns all new state tensors plus (newly_done, is_success, finished_idx).
            """
            # 1. Get actions
            ...
            # 4. Reset - get new queries and derived states
        ...
        print(f"[MinimalEvalV5] Compiled (mode={mode})")
    ...
    def evaluate(
        ...
        while steps < max_steps:
            ...
            # Call compiled step with current state
            (
            ...
            # Copy results OUTSIDE compiled step (avoids CUDA graph mutation issue)
            self._current.copy_(new_current)
            self._derived.copy_(new_derived)
            self._counts.copy_(new_counts)
