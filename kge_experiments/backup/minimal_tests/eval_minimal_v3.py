"""
Minimal Eval V3 - Uses torch.compiler.disable() to avoid tracing engine.

Key: Use @torch.compiler.disable on engine call to prevent nested tracing.
"""
import torch
...
class MinimalEvalV3:
    """Minimal eval with step+reset compiled, engine call disabled from tracing."""
    ...
    def _get_derived_untraced(self, queries: Tensor) -> Tuple[Tensor, Tensor]:
        """Call engine without being traced by torch.compile."""
        return self.engine.get_derived_states_compiled(queries, self._var_buf, None)
    
    def _compute_derived(self, queries: Tensor) -> Tuple[Tensor, Tensor]:
        """Compute derived for [B, A, 3] queries, padding to static shape."""
        B, S, A, pad = self.B, self.S, self.A, self.pad
        
        derived_raw, counts_raw, _ = self._get_derived_untraced(queries)
    ...
    def _init_from_pool(self):
        ...
        derived, counts = self._compute_derived(queries)
    ...
    def compile(self, mode: str = 'reduce-overhead'):
        """Compile step+reset with engine call disabled from tracing."""
        policy = self.policy
        B, S, A, pad = self.B, self.S, self.A, self.pad
        ...
        def step_and_reset() -> Tuple[Tensor, Tensor, Tensor]:
            """Step + auto-reset. Engine call is not traced."""
            # 1. Get actions
            ...
            # 4. Reset (engine call untraced via @torch.compiler.disable)
            ...
            # This call exits the traced graph via @torch.compiler.disable
            reset_derived, reset_counts = eval_obj._compute_derived(reset_queries)
        ...
        print(f"[MinimalEvalV3] Compiled (mode={mode})")
