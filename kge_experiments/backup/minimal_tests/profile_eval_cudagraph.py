"""
Profile Manual CUDA Graph Evaluation.
"""
import os
import sys
...
def setup(device, config):
    ...
    from eval_cudagraph import EvalCUDAGraph
...
def main():
    ...
    print(f"Manual CUDA Graph Eval | Candidates: {total}")
    
    print("Setup...")
    c = setup(device, config)
    
    print("Warmup + graph capture...")
    t0 = time()
    c['eval'].reset(c['queries'][:5].to(device))
    c['eval'].capture_graph()
    ...
    print(f"ms/candidate: {ms:.4f}  (target: 0.03)")
