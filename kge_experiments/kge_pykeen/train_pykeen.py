...
def _supports_complex_cuda(device: str) -> bool:
    """Return True if complex tensor ops work on the requested CUDA device."""
    if not device.startswith("cuda"):
        return True
    try:
        _ = torch.abs(torch.randn(8, device=device, dtype=torch.complex64))
        return True
    except Exception:
        return False
...
def train_model(
    ...
    Args:
        ...
        training_config: Training configuration dict with keys:
            ...
            - early_stop_metric: Metric name for early stopping (default: inverse_harmonic_mean_rank)
            - early_stop_delta: Minimum relative delta for early stopping (default: 0.0)
    ...
    early_stop_metric = training_config.get('early_stop_metric', 'inverse_harmonic_mean_rank')
    early_stop_delta = float(training_config.get('early_stop_delta', 0.0))

    if model_name in {"RotatE", "ComplEx"} and not _supports_complex_cuda(device):
        raise RuntimeError(
            "CUDA complex ops are not available (nvrtc builtins missing). "
            "Use a real-valued model (e.g., TransE/DistMult) on GPU or fix the CUDA install."
        )
    ...
    if use_early_stopping:
        ...
        stopper_kwargs = {
            ...
            "relative_delta": early_stop_delta,
            "larger_is_better": True,
        }
        if verbose:
            print(f"Early stopping enabled: metric={early_stop_metric}, patience={patience}")
            print()
    ...
    if save_dir is not None:
        ...
        model_subdir = save_dir / (run_signature or f"{dataset_name}_{model_name}")
