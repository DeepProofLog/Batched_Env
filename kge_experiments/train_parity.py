...
def create_compiled_components(config: TrainConfig) -> Dict[str, Any]:
    ...
    # Create vectorized engine (always use optimized path)
    vec_engine = UnificationEngineVectorized.from_index_manager(
        im,
        padding_atoms=config.padding_atoms,
        parity_mode=getattr(config, 'parity', False),
    ...
    test_queries_tensor = im.queries_to_tensor(dh.test_queries, device)
...
def run_experiment(config: TrainConfig, return_traces: bool = False) -> Dict[str, Any]:
    ...
    # Create PPO (always use PPOOptimized)
    print("\n[2/3] Running training...")
    seed_all(config.seed, deterministic=getattr(config, 'parity', False))
