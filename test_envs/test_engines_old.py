...
def test_single_query(p: str, h: str, t: str, 
    ...
    for step in range(max_depth):
        ...
        derived, derived_counts, next_var_tracker = engine.get_derived_states(
            ...
            excluded_queries=query_padded, verbose=0, # if verbose else 0,
...
def main():
    ...
    # Removed 'start_query' argument - we run tests on all selected queries
    ...
    # Deprecated: 'start_query' was removed; always start from the first query
    ...
    all_queries = all_queries[:args.n_queries] if args.n_queries is not None else all_queries
    ...
    print(f"  Will test {queries_to_test} queries (indices 0 to {total_queries-1})")
    print()
    ...
    for query_idx, (split, p, h, t) in enumerate(all_queries):
        ...
        match, str_success, tensor_success, reason = test_single_query(
            p, h, t, str_engine_data, tensor_engine_data, facts_set_str, verbose=verbose,
