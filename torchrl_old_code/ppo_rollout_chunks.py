...
def collect_rollouts(
    ...
    if not batch_critic:
        ...
        if debug:
            print(f"\n[collect_rollouts] Evaluating critic on entire batch at once")
            print(f"  Batch size: {batch_td.batch_size}")
        critic(batch_td)  # adds 'state_value' in-place
        if debug:
            print(f"[collect_rollouts] Critic evaluation complete")
    else:
        # Add value estimates using the critic (operates on the observation at ROOT)
        # CRITICAL FIX: batch_td has shape [n_envs, n_steps], need to flatten to [n_envs*n_steps]
        # before chunking to avoid processing entire n_steps at once
        
        print(f"\n{'='*80}")
        print(f"[MEMORY PROFILING] Starting critic evaluation")
        print(f"  Original batch_td.batch_size: {batch_td.batch_size}")
        
        # Flatten from [n_envs, n_steps] to [n_envs * n_steps]
        batch_td_flat = batch_td.reshape(-1)
        print(f"  Flattened batch_td.batch_size: {batch_td_flat.batch_size}")
        print(f"{'='*80}\n")
        ...
        print(f"  Number of chunks: {(total_size + chunk_size - 1) // chunk_size}")
        
        # Pre-allocate state_value list on CPU
        state_values_list = []
        
        for i, start_idx in enumerate(range(0, total_size, chunk_size)):
            ...
            if (i + 1) % 50 == 0 or (i + 1) == (total_size + chunk_size - 1) // chunk_size:
                print(f"  Processed {i + 1}/{(total_size + chunk_size - 1) // chunk_size} chunks...")
        ...
        batch_td = batch_td_flat.reshape(n_envs, n_steps)
        print(f"\nCritic evaluation completed! Final batch_td.batch_size: {batch_td.batch_size}\n")
