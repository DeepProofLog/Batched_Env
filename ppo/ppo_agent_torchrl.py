...
class PPOAgentTorchRL:
    ...
    def _debug_print_rollout_stats(self, experiences: List[TensorDict], stats: Dict[str, Any]) -> None:
        ...
        if isinstance(episode_info, dict):
            ...
            lengths = episode_info.get("length")
            if rewards is not None and rewards.numel() > 0:
                ep_return = rewards[-1].detach().cpu().item()
                ep_len = lengths[-1].detach().cpu().item() if lengths is not None and lengths.numel() > 0 else None
                print(
                    f"    [debug] Last episode stats: return={ep_return} length={ep_len} tensor_keys={list(episode_info.keys())}"
                )
        elif episode_info:
            last_episode = episode_info[-1]
    ...
    def _grad_norm(self, params: List[nn.Parameter]) -> torch.Tensor:
        """Compute global L2 norm for a parameter collection. Returns a tensor (not a scalar)."""
        # Collect all gradient norms as tensors and compute on GPU
        grad_norms = []
        for p in params:
        ...
        return torch.norm(torch.stack(grad_norms), 2)

    def _sum_loss_terms(self, loss_out: TensorDict) -> torch.Tensor:
        """Sum every `loss_*` tensor returned by TorchRL."""
        total_loss = None
        for key in loss_out.keys():
            if isinstance(key, str) and key.startswith("loss_"):
                component = loss_out.get(key)
                total_loss = component if total_loss is None else total_loss + component
        if total_loss is None:
            available = ", ".join(str(k) for k in loss_out.keys())
            raise RuntimeError(
                f"ClipPPOLoss returned no loss_* entries. Available keys: [{available}]"
            )
        return total_loss
    ...
    def _get_first_key(
        ...
        *,
        ...
        """Return the first present key from `candidate_keys`, optionally enforcing presence."""
        ...
        if required_label is not None:
            available = ", ".join(str(k) for k in loss_out.keys())
            raise RuntimeError(
                f"ClipPPOLoss output missing {required_label}. Available keys: [{available}]"
            )
        return default

    @staticmethod
    def _scalar_to_float(value: Any) -> float:
        """Convert a tensor or scalar-like value to a Python float for logging."""
    ...
    def _debug_print_optimizer_stats(self, metrics: Dict[str, float]) -> None:
        ...
        pol = self._scalar_to_float(metrics.get("policy_loss", 0.0))
        val = self._scalar_to_float(metrics.get("value_loss", 0.0))
        ent = self._scalar_to_float(metrics.get("entropy", 0.0))
        kl = self._scalar_to_float(metrics.get("approx_kl", 0.0))
        clip = self._scalar_to_float(metrics.get("clip_fraction", 0.0))
        ev = self._scalar_to_float(metrics.get("explained_variance", 0.0))
        print(
        ...
        if "grad_actor_norm" in metrics:
            actor = self._scalar_to_float(metrics.get("grad_actor_norm", 0.0))
            critic = self._scalar_to_float(metrics.get("grad_critic_norm", 0.0))
            total = self._scalar_to_float(metrics.get("grad_total_norm", 0.0))
            print(
    ...
    def learn(
        ...
        batch_size_int = int(self.batch_size)  # Cache the conversion
        num_minibatches = max(1, flat_bs // batch_size_int)
        print(f"flat_bs={flat_bs}, batch_size={self.batch_size}, num_minibatches={num_minibatches},")
        indices = torch.arange(flat_bs, device=self.device)

        # Use tensor accumulators on GPU to avoid repeated .item() calls
        ...
        for epoch in range(self.n_epochs):
            ...
            for mb in range(num_minibatches):
                start_idx = mb * batch_size_int
                end_idx = start_idx + batch_size_int
                ...
                policy_loss = self._get_first_key(
                    loss_out,
                    ("loss_objective", "loss_policy", "loss_actor", "loss"),
                    required_label="policy loss term",
                )
                value_loss = self._get_first_key(
                    loss_out,
                    ("loss_critic",),
                    default=zero_tensor,
                )
                entropy_metric = self._get_first_key(
                    loss_out,
                    ("entropy",),
                ...
                if num_updates < 3 and self.verbose:  # Modified debug print
                    print(
                        "[AGENT]  DEBUG loss:"
                        f" epoch={epoch} mb={mb} pg={self._scalar_to_float(policy_loss):.6f}"
                        f" v={self._scalar_to_float(value_loss):.6f}"
                        f" ent={self._scalar_to_float(entropy_metric):.6e}"
                    )

                self.optimizer.zero_grad(set_to_none=True)
                actor_grad_val: Optional[float] = None
                critic_grad_val: Optional[float] = None
                total_norm_val: Optional[float] = None
                if self.use_amp:
                    self.grad_scaler.scale(total_loss).backward()
                    self.grad_scaler.unscale_(self.optimizer)
                    if all_params:
                        if self.debug_mode:
                            actor_grad_val = self._scalar_to_float(self._grad_norm(actor_params))
                            critic_grad_val = self._scalar_to_float(self._grad_norm(critic_params))
                        total_norm_val = self._scalar_to_float(
                ...
                if self.debug_mode and all_params and total_norm_val is not None:
                    grad_actor_sum += actor_grad_val if actor_grad_val is not None else 0.0
                    grad_critic_sum += critic_grad_val if critic_grad_val is not None else 0.0
                ...
                epoch_entropy += entropy_metric.detach()
                epoch_approx_kl += approx_kl.detach()
                epoch_clip_fraction += clip_fraction.detach()
                epoch_updates += 1
                
                if self.verbose:
                    # Only transfer for debug printing
                    print(
                        "[AGENT]  DEBUG Epoch {epoch}, Minibatch {mb} loss: pg={pl:.6f}, v={vl:.6f}, ent={ent:.6f}".format(
                            pl=self._scalar_to_float(policy_loss),
                            vl=self._scalar_to_float(value_loss),
                            ent=self._scalar_to_float(entropy_metric),
                        )
                    )
                ...
                total_entropy += entropy_metric.detach()
            ...
            if epoch_updates > 0:
                ...
                if self.debug_mode:
                    epoch_values = epoch_metrics.detach().cpu().tolist()
                    epoch_pl, epoch_vl, epoch_ent, epoch_kl, epoch_clip = epoch_values
                    grad_str = ""
                    if grad_measurements > 0:
                        avg_grad = self._scalar_to_float(grad_total_sum / grad_measurements)
                        grad_str = f", grad_norm={avg_grad:.6f}"
                    print(
                        "[AGENT]  Epoch {idx}. Losses: pg={pl:.6f}, v={vl:.6f}, ent={ent:.6f}, kl={kl:.6f}, clip={clip:.6f}{grad}".format(
                            idx=epoch + 1,
                            pl=epoch_pl,
                            vl=epoch_vl,
                            ent=epoch_ent,
        ...
        aggregated_metrics = torch.stack(
            (
                total_policy_loss,
                total_value_loss,
                total_entropy,
        ...
        policy_loss, value_loss, entropy, approx_kl, clip_fraction = aggregated_metrics.detach()
        ...
        with torch.no_grad():
            ...
            explained_var = 1 - (var_residual / (var_vtarget + 1e-8))
    ...
    def train(
            ...
            for iteration in range(n_iterations):
                ...
                episode_info = stats.get("episode_info")
                has_episode_stats = False
                episode_count = 0
                if isinstance(episode_info, dict):
                    reward_tensor = episode_info.get("reward")
                    has_episode_stats = reward_tensor is not None and reward_tensor.numel() > 0
                    if reward_tensor is not None:
                        episode_count = int(reward_tensor.shape[0])
                elif episode_info:
                    has_episode_stats = True
                    episode_count = len(episode_info)

                if callback_manager is not None and has_episode_stats:
                    if self.verbose_cb:
                        print(f"[PPOAgent] Accumulating {episode_count} episode stats for training")
                ...
                if logger is not None:
                    logger.log_training_step(
                        ...
                        policy_loss=self._scalar_to_float(train_metrics["policy_loss"]),
                        value_loss=self._scalar_to_float(train_metrics["value_loss"]),
                        entropy=self._scalar_to_float(train_metrics["entropy"]),
                    )
    ...
    def _save_checkpoint(
        ...
        checkpoint = {
            'actor_state_dict': self.actor.module.state_dict() if hasattr(self.actor, 'module') else self.actor.state_dict(),
            'critic_state_dict': self.critic.module.state_dict() if hasattr(self.critic, 'module') else self.critic.state_dict(),
