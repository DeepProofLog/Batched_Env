...
from index_manager_idx import IndexManager # Assuming this is defined in index_manager.py

# --- Utility to check if a string is a variable (user-defined convention) ---
def is_user_variable_str(arg_s: str) -> bool:
    """Checks if a string argument represents a user-level variable."""
    return arg_s and (arg_s[0].isupper() or arg_s[0] == '_')
...
def term_to_tensor_im(term_str: Term, index_manager: IndexManager, var_map: Dict[str, int]) -> torch.Tensor:
    pred_idx = index_manager.get_predicate_idx_by_str(term_str.predicate)
    arg_indices = []
    ...
    for arg_s in term_args:
        if is_user_variable_str(arg_s): 
            if arg_s not in var_map:
                var_map[arg_s] = index_manager.get_fresh_dynamic_var_idx()
            arg_indices.append(var_map[arg_s])
        else: 
            arg_indices.append(index_manager.get_constant_idx_by_str(arg_s))
...
def state_to_tensor_im(state_str: List[Term], index_manager: IndexManager, initial_var_map: Optional[Dict[str, int]] = None) -> torch.Tensor:
    ...
    if initial_var_map is None: 
        for term_obj in state_str:
            ...
            for arg_s in term_args:
                if is_user_variable_str(arg_s) and arg_s not in current_var_map:
                    current_var_map[arg_s] = index_manager.get_fresh_dynamic_var_idx()
        ...
        for k in range(1, index_manager.max_arity + 1): 
            ...
            if is_variable_idx(original_arg_idx, index_manager): 
                if original_arg_idx not in local_original_var_to_fresh_dynamic_map:
                    local_original_var_to_fresh_dynamic_map[original_arg_idx] = index_manager.get_fresh_dynamic_var_idx() 
...
def debug_print_atom(atom_tensor: torch.Tensor, index_manager: IndexManager) -> str: 
    ...
    pred_str = index_manager.get_str_for_pred_idx(pred_idx_val)
    args_str_list = [] 
    for i in range(1, index_manager.max_arity + 1): 
